        ...
        {hasOptions && isLatest && (
          <div className="mt-3 flex flex-col gap-1.5 border-t border-white/15 pt-3">
            {message.options!.map((opt) => {
              const clickable = !opt.selected && !!onOptionSelect;
              return (
                <button
                  key={opt.id}
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-xs transition-all border ${
                    opt.selected
                      ...
                      : clickable
                        ? 'border-white/20 hover:border-cm-primary/40 hover:bg-cm-primary/10 text-white/80 hover:text-white cursor-pointer shadow-cm-ambient'
                        : 'border-white/15 text-white/50 cursor-default'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <span className={`shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                      opt.selected ? 'bg-cm-primary/30 text-cm-primary' : 'bg-white/15 text-white/60'
                    }`}>
