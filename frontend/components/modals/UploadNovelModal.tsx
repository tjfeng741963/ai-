...
export function UploadNovelModal({ open, onClose }: UploadNovelModalProps) {
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={handleClose}
              className="absolute top-4 right-4 p-2 text-slate-400 hover:text-cm-primary transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 rounded-2xl bg-cm-primary/10 flex items-center justify-center border border-cm-primary/20">
                <CloudUpload className="w-6 h-6 text-cm-primary" />
              </div>
              <div>
                <h2 className="text-xl font-headline font-bold text-cm-on-surface">上传小说</h2>
            ...
            <div
              className={`relative border-2 border-dashed rounded-2xl p-10 text-center transition-all duration-500 ${
                dragOver
                  ...
                  : file
                  ? 'border-cm-primary/30 bg-cm-primary/5'
                  : 'border-cm-outline-variant/30 hover:border-cm-primary/30'
              }`}
              onDragOver={(e) => {
                e.preventDefault();
                setDragOver(true);
            ...
            >
              {file ? (
                <div className="flex flex-col items-center gap-3">
                  <FileText className="w-10 h-10 text-cm-primary" />
                ...
                <div className="flex flex-col items-center gap-3">
                  <CloudUpload className="w-10 h-10 text-slate-500 animate-cm-float" />
                  <p className="text-cm-on-surface-variant font-body">
                    拖拽文件到此处，或{' '}
                    <label className="text-cm-primary cursor-pointer hover:underline">
                      点击选择
                      <input
                        type="file"
                        accept=".txt,.md"
                  ...
                  <p className="text-xs text-slate-500">支持 TXT、Markdown 格式</p>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-full bg-gradient-to-r from-cm-primary to-cm-primary-dim text-cm-on-primary text-sm font-bold shadow-cm-neon hover:scale-105 transition-all duration-500 disabled:opacity-40 disabled:hover:scale-100"
              >
                {loading ? (
